...
namespace NGLinkingPin
{
    ...
    public sealed class FabricatorPatch : IAssemblyPlugin
    {
        ...
        {
            if (!_suppressedFabs.TryGetValue(__instance, out var removed)) return;
            _suppressedFabs.Remove(__instance);

            foreach (var item in removed) { __instance.Item.linkedTo.Add(item); }

            if (_availableIngredientsField == null || removed.Count == 0) return;
            ...
            foreach (var linkedItem in removed)
            {
                ...
                {
                    AddToAvailable(available, containedItem);
                    var nested = containedItem.GetComponent<ItemContainer>();
                    if (nested != null)
                    {
